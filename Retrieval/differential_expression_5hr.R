# CFC retrieval differential expression analysis 5 hour
...
setwd("~/Documents/PhD/Retrieval_5hr/featurecount_files/")

# set variables

n_group <- 12 # how many samples per group

output_path <- "~/Documents/PhD/Retrieval_5hr/DGE_results/"

design_formula <- "~ 0 + group + RIN + weight"
p_cutoff <- 0.1
...
outliers <- as.character("0")#as.character(c("6", "14","21"))

samples <- samples[!samples %in% outliers]

files <- c(paste0("CFC_5hr_", samples, ".markdup.featurecount")) # file names
...
setwd("~/Documents/PhD/Retrieval_5hr/DGE_results/")
...
sample_names <- str_extract(x$samples$files, "CFC_5hr_[1-9][0-9]?")
...
sample_info <- read_delim("../sample_characteristics.txt", delim = "\t") %>%
...
x$samples$weight <- sample_info$weight
...
contr.matrix <- makeContrasts(
  norecallvsRecall = grouprecall - groupno_recall,
  norecallvsExtinction = groupextinction - groupno_recall,
